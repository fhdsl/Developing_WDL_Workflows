...
<pre><code>task cowsay {
  ...
  command <<<
    cowsay -t <span style="color:red;">"~{some_string}"</span>
  >>>
}
</code></pre>
...
Thankfully, the solution here is simple: _Move all of the input files directly into the working directory_.
...
task BwaMem {
  ...
  # <span style="color:red;">basename()</span> is a built-in WDL function that acts like bash's basename
...
If you are writing WDLs under the WDL 1.1 standard, you may have more flexibility with runtime attributes. **_Beware_** that as of February 2024, Cromwell does not support WDL 1.1.

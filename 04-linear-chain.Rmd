...
The File variables `analysisReadyBam` and `analysisReadySorted` can now be accessed anywhere in the workflow block after the `BwaMem` task as `BwaMem.``analysisReadyBam` and `BwaMem.analysisReadySorted`, respectively. Then, we will take the output of `BwaMem` task into the input of the `MarkDuplicates` task: we call our next task `MarkDuplicates` with input `BwaMem.analysisReadySorted` from the `BwaMem` task:

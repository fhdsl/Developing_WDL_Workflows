...
For instance, in this dummy `LinearChain` workflow below, after the task `stepA` is called, its task output `out` can be accessed at the workflow level as inputs for the task `stepB` in the form of `stepA.out`. This example is adapted from [OpenWDL Docs](https://docs.openwdl.org/en/latest/WDL/Linear_chaining/)' introduction to linear chain, which has more [introductory examples and variations of writing linear chains](https://docs.openwdl.org/en/latest/WDL/add_plumbing/).
...
Now let's see how this applies to our workflow: we will take the output of `BwaMem` task into the input of the `MarkDuplicates` task. Recall that in the `BwaMem` task, the **output** section of the task is the following:
...
The File variables `analysisReadyBam` and `analysisReadySorted` can now be accessed anywhere in the workflow block after the `BwaMem` task as `BwaMem.``analysisReadyBam` and `BwaMem.analysisReadySorted`, respectively. Therefore, we call our next task `MarkDuplicates` with input `BwaMem.analysisReadySorted`:
...
Similar to the `BwaMem` task, we will need to specify output files as Strings, relative to the working directory. We will name our output files based on the base name of the original input file in the workflow. Therefore, it makes sense to create a private String variable `base_file_name` that contains this base name based on `input_bam`. We will use `base_file_name` downstream in the Command section to specify the output bam and metrics files.
...
As we have been doing all along, we refer to those any defined variables from the input or private variables using \~{this} syntax.
